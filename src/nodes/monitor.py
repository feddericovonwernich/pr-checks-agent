...
from state.schemas import MonitorState, CheckStatus
...
def should_analyze_failures(state: MonitorState) -> str:
    ...
    active_prs = state.get("active_prs", {})
    needs_analysis = [
        pr_num for pr_num, pr_state in active_prs.items()
        if pr_state.get("workflow_step") == "needs_analysis"
    ]
...
async def prioritize_failures(state: MonitorState) -> Dict[str, Any]:
    ...
    return {
        **state,
        "prioritized_failures": prioritized_checks
    }
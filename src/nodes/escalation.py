...
from state.schemas import EscalationRecord, EscalationStatus, MonitorState
...
async def escalation_node(state: MonitorState) -> dict[str, Any]:
    ...
    for candidate in escalation_candidates:
        ...
        try:
            ...
            escalation_results.append({"pr_number": pr_number, "check_name": check_name, "success": False, "error": str(e)})

    # Summary logging
    successful_escalations = sum(1 for result in escalation_results if result["success"])
    logger.info(f"Escalation complete: {successful_escalations}/{len(escalation_results)} successful")
    ...
    for pr_number, pr_state in active_prs.items():
        ...
        for check_name in failed_checks:
            ...
            if len(attempts) >= max_attempts:
                ...
                all_failed = all(attempt.get("status") == "failure" for attempt in attempts)
...
def _is_in_cooldown(pr_state: dict[str, Any], check_name: str, cooldown_hours: int) -> bool:
    ...
    check_escalations = [escalation for escalation in escalations if escalation.get("check_name") == check_name]

    if not check_escalations:
        return False

    # Get the most recent escalation
    latest_escalation = max(check_escalations, key=lambda x: x.get("timestamp", datetime.min))
...
async def handle_escalation_response(
    state: MonitorState, escalation_id: str, response_type: str, user_id: str, notes: str = ""
) -> dict[str, Any]:
    ...
    return {**state, "active_prs": updated_prs}

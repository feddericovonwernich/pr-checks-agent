...
from state.schemas import MonitorState, FixAttempt, FixAttemptStatus
...
async def claude_invoker_node(state: MonitorState) -> Dict[str, Any]:
    ...
    for issue in fixable_issues:
        ...
        if len(check_attempts) >= max_attempts:
            logger.warning(
                f"Max fix attempts ({max_attempts}) reached for {check_name} "
                f"in PR #{pr_number}, skipping"
            )
            continue
        
        logger.info(
            f"Attempting fix {len(check_attempts) + 1}/{max_attempts} "
            f"for {check_name} in PR #{pr_number}"
        )
        ...
        try:
            ...
            fix_attempt_dict.update({
                ...
                "status": FixAttemptStatus.SUCCESS.value if fix_result.get("success", False) else FixAttemptStatus.FAILURE.value,
            ...
            if check_name in fix_attempts and fix_attempts[check_name]:
    ...
    logger.info(
        f"Fix attempts complete: {successful_fixes}/{len(fix_results)} successful"
    )
    ...
    return {
        **state,
        ...
        "fix_stats": {
            ...
            "successful_count": successful_fixes,
            "timestamp": datetime.now()
        }
    }

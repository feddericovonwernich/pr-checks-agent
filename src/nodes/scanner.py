...
async def repository_scanner_node(state: MonitorState) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Repository scan results: {len(new_pr_numbers)} new, "
            f"{len(updated_prs)} updated, {len(closed_prs)} closed PRs"
        )
...
def should_continue_scanning(state: MonitorState) -> str:
    ...
    if new_prs or updated_prs:
        logger.info(f"Found changes, proceeding to check monitoring")

...
class StatePersistence:
    ...
    def load_monitor_state(self, repository: str) -> Optional[MonitorState]:
    ...
    def load_pr_state(self, repository: str, pr_number: int) -> Optional[PRState]:
        """Load PR state."""
        try:
            key = f"pr_state:{repository}:{pr_number}"
            data = self.redis_client.get(key)
    ...
    def set_counter(self, key: str, value: int, ttl: Optional[int] = None) -> bool:
        """Set a counter value."""
        try:
            if ttl:

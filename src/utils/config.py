...
class Config(BaseModel):
    ...
    def validate_environment(self) -> dict[str, Any]:
        ...
        optional_vars = ["REDIS_URL", "WEBHOOK_SECRET"]
            ...
            if repo.fix_limits.get("escalation_enabled", True):
                if not os.getenv("TELEGRAM_BOT_TOKEN"):
                    validation_results["warnings"].append(
                        f"Repository {repo.owner}/{repo.repo} has escalation enabled but TELEGRAM_BOT_TOKEN not set"
                    )
        ...
        repositories=[
            RepositoryConfig(
                ...
                priorities={
                    ...
                    "branch_priority": {"main": 1, "develop": 2, "feature/*": 3},
                },
                notifications={"telegram_channel": "@dev-alerts", "escalation_mentions": ["@dev-lead"]},
            )
        ],
...
def validate_config_file(config_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        validation_results["stats"] = {
            ...
            "total_branches_monitored": sum(len(repo.branch_filter) for repo in config.repositories),
            "escalation_enabled_repos": sum(
                1 for repo in config.repositories if repo.fix_limits.get("escalation_enabled", True)
            ),
        }
        ...
        for repo in config.repositories:
            ...
            if max_attempts > 10:
                validation_results["warnings"].append(f"Repository {repo_key} has high max_attempts: {max_attempts}")

        # Validate global limits
        if config.global_limits.max_daily_fixes > 200:
            validation_results["warnings"].append(f"High daily fix limit: {config.global_limits.max_daily_fixes}")

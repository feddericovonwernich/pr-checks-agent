...
FIX_ATTEMPTS_TOTAL = Counter(
    'pr_agent_fix_attempts_total',
    'Total number of fix attempts',
    ['repository', 'check_type', 'success'],
    registry=REGISTRY
)

ESCALATIONS_TOTAL = Counter(
    'pr_agent_escalations_total',
    'Total number of escalations sent',
    ['repository', 'reason', 'success'],
    registry=REGISTRY
)

# Histograms
FIX_DURATION_SECONDS = Histogram(
    'pr_agent_fix_duration_seconds',
    'Time spent on fix attempts',
    ['repository', 'check_type'],
    registry=REGISTRY
)

GITHUB_API_DURATION_SECONDS = Histogram(
    'pr_agent_github_api_duration_seconds',
...
ACTIVE_PRS = Gauge(
    'pr_agent_active_prs',
    'Number of active PRs being monitored',
    ['repository'],
    registry=REGISTRY
)

WORKFLOW_ERRORS = Gauge(
    'pr_agent_workflow_errors',
    'Number of consecutive workflow errors',
    ['repository'],
    registry=REGISTRY
)
...
class MonitoringServer:
    ...
    async def health_check(self, request: web.Request) -> web.Response:
        ...
        health_data = {
            ...
            "recent_errors": len([
                event for event in self.stats["recent_events"][-10:]
    ...
    async def api_stats(self, request: web.Request) -> web.Response:
        ...
        stats_data = {
            ...
            "total_active_prs": sum(
                repo_stats.get("active_prs", 0) 
                for repo_stats in self.stats["repositories"].values()
            ),
...
def record_fix_attempt(repository: str, check_type: str, success: bool, duration: float):
    ...
    FIX_ATTEMPTS_TOTAL.labels(
        repository=repository, 
        check_type=check_type, 
        success=success_label
    ).inc()
    
    FIX_DURATION_SECONDS.labels(
        repository=repository, 
        check_type=check_type
    ).observe(duration)

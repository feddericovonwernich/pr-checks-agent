...
def create_monitor_graph(
    ...
    graph.add_conditional_edges(
        ...
        should_analyze_failures,
        {
            "analyze_failures": "prioritize_failures",
            "wait_for_next_poll": "wait_for_poll"
        }
    )
    ...
    graph.add_conditional_edges(
        ...
        {
            "wait_for_human_response": "wait_for_poll",
            "wait_for_next_poll": "wait_for_poll"
        }
    )
...
async def _wait_for_next_poll(state: MonitorState) -> Dict[str, Any]:
    ...
    return {
        **state,
        "workflow_step": "ready_for_next_poll",
...
async def _cleanup_old_state(state: MonitorState) -> Dict[str, Any]:
    ...
    return {
        **state,
        "active_prs": cleaned_prs,
        "cleanup_stats": {
            "prs_removed": removed_count,

...
class GitHubAPIInput(BaseModel):
    ...
    pr_number: Optional[int] = Field(default=None, description="PR number for PR-specific operations")
    check_run_id: Optional[int] = Field(default=None, description="Check run ID for log retrieval")
    branch_filter: Optional[List[str]] = Field(default=None, description="Filter PRs by branches")
...
class GitHubTool(BaseTool):
    ...
    async def _get_open_prs(self, repository: str, branch_filter: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    def _map_check_status(self, status: str, conclusion: Optional[str]) -> CheckStatus:
